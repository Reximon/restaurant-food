<?php require_once '../objects/includes/header.php'; ?>
<div class="main">
        <div class="container a-container" id="a-container">
                    <div class="back">
                        <a href="../index.php"> <i class="fas fa-times"></i></a>
                    </div>
                    
            <form class="form" id="a-form" method="POST" action="..\objects/helpers/registro.php">
                <h2 class="form_title title">Crear Cuenta</h2>
                <?php 
                    if(isset($_SESSION['completado'])): ?>
                    <div class="alerta alerta-exito">
                        <?= $_SESSION['completado']; ?>
                    </div>
                    <?php elseif(isset($_SESSION['errores'] ['general'])): ?>
                        <div class="alerta alerta-exito">
                        <?= $_SESSION['errores'] ['general']; ?>
                    </div>
                    <?php endif; ?>
                <div class="form__icons">
                <img class="form__icon" src="data:image/svg+xml;base64," alt="">
                <img class="form__icon" src="data:image/svg+xml;base64,">
                <img class="form__icon" src="data:image/svg+xml;base64,"></div>
                <span class="form__span">o usa un email para el registro</span>
                <input class="form__input alerta-error" name="name" type="text" placeholder="Introduce tu Nombre" required>
                <?php echo isset($_SESSION['errores']) ? mostrarError($_SESSION['errores'], 'nombre') : ''; ?>
                
                <input class="form__input alerta-error" name="apellidos" type="text" placeholder="Introduce tus Apellidos" required>
                <?php echo isset($_SESSION['errores']) ? mostrarError($_SESSION['errores'], 'apellidos') : ''; ?>
                
                <input class="form__input alerta-error" name="email" type="text" placeholder="Introduce tu Email" required>
                <?php echo isset($_SESSION['errores']) ? mostrarError($_SESSION['errores'], 'email') : ''; ?>
                
                <input class="form__input alerta-error" name="password" type="password" placeholder="Introduce tu Contraseña" required>
                <?php echo isset($_SESSION['errores']) ? mostrarError($_SESSION['errores'], 'contraseña') : ''; ?>
                <!-- <p class="form__utilities">Utiliza ocho caracteres como mínimo con una combinación de letras, números y símbolos.</p> -->
                <div class="checkbox-cx">
                    <input type="checkbox" id="horns" required><label for="horns">He leído y acepto las Condiciones de uso.</label>
                </div>

                <button class="formulario__button button" type="submit">Registrarse</button>
            </form>
        </div>
        <div class="container b-container" id="b-container">
                <div class="back-b">
                    <a href="../index.php"> <i class="fas fa-times"></i></a>
                </div>
                
            <form class="form" id="b-form" method="POST" action="..\objects/helpers/loginValidate.php">
                <h2 class="form_title title">Iniciar sesion</h2>
                <?php if(isset($_SESSION['error_login'])): ?>
                        <div class="alerta alerta-login">
                            <?= $_SESSION['error_login']?>
                        </div>
                <?php endif; ?>
                <div class="form__icons">
                    <img class="form__icon" src="data:image/svg+xml;base64," alt="">
                    
                    <img class="form__icon" src="data:image/svg+xml;base64,">
                    
                    <img class="form__icon" src="data:image/svg+xml;base64,">
                </div>
                <span class="form__span">o usa tu email</span>
                <input class="form__input alerta-error" name="email" type="text" placeholder="Introduce tu Email" required>
                
                <div class="field space">
                    <input class="form__input alerta-error" name="password" type="password" placeholder="Introduce tu Contraseña" id="password">
                    
                    <span id="loginShow" class="passwordIcon"><i class="fas fa-eye"></i> </span>
                    <span id="loginHide" class="passwordIcon"> <i class="fas fa-eye-slash"></i> </span>
                </div>
                <a href="#" class="form__link"> ¿Has olvidado tu contraseña?</a>
                <button class="formulario__button button" type="submit">Iniciar Sesión</button>
            </form>
            <?php borrarErrores();?>
        </div>
        <div class="switch" id="switch-cnt">
            <div class="switch__circle"></div>
            <div class="switch__circle switch__circle--t"></div>
            <div class="switch__container" id="switch-c1">
                <h2 class="switch__title title">¡Hola de nuevo!</h2>
                <p class="switch__description description">Conecta con tus creedenciales para hacer un pedido</p>
                <button class="switch__button button switch-btn">Iniciar Sesión</button>
            </div>
            <div class="switch__container is-hidden" id="switch-c2">
                <h2 class="switch__title title">¿Sin cuenta?</h2>
                <p class="switch__description description">Crea una cuenta para poder hacer tus pedidos en ƒood</p><button class="switch__button button switch-btn">Crear Cuenta</button>
            </div>
        </div>
    </div>
    <script src="..\assets/js/login-main.js"></script>
    <script>
        const show = document.querySelector('#loginShow');
        const hide = document.querySelector('#loginHide');
        const idPassword = document.querySelector('#password');
        
        show.addEventListener('click', function(){ 
            show.style.display = "none";
            hide.style.display = "block";
            idPassword.type = "password";
        });
        hide.addEventListener('click', function(){ 
            hide.style.display = "none";
            show.style.display = "block";
            idPassword.type = "text";
        });
    </script>
</body>
<footer>
    <a href="index.php" class="logo">ƒood<span>.</span></a>
</footer>
</html>